...
namespace spec {
    ...
    void GlassnerUpsampler::upsample_pixel_to(const Pixel &pixel, BasicSpectrum &spectre) const
    {
        vec3 ampls = rgb2xyz(pixel.to_vec3()) * XYZ_TO_SPECTRE_INV;
        spectre = BasicSpectrum();
    ...
    void GlassnerUpsampler::upsample(const Image &sourceImage, BasicSpectralImage &dest) const
    {
        init_progress_bar();
